...
my ($fasta_file, $bin_name, $out_file, $seed_file, $md5, $log_file, $tmp_dir);
my $max_iter     = 10;
my $min_converge = 3;
my $uclust_id    = 0;
...
if ( ! GetOptions( "fasta_file=s"    => \$fasta_file,
		   ...
		   "tmp_dir=s"       => \$tmp_dir,
		   "max_iter:i"      => \$max_iter,
		   "min_converge:i"  => \$min_converge,
		   "uclust_id:i"     => \$uclust_id,
		 )
   ) { &usage; }
...
ML: for ($i = 1; $i <= $max_iter; $i++) {
  ...
  #system("usearch --sort $fa_file --output $fa_file.sort > /dev/null 2>&1");
  #$log_text .= `usearch --cluster $fa_file.sort --uc $uc_file --id $uclust_id --rev 2>&1`;
  system("uclust --sort $fa_file --output $fa_file.sort > /dev/null 2>&1");
  $log_text .= `uclust --nucleo --input $fa_file.sort --uc $uc_file --id $uclust_id --rev 2>&1`;
...
&write_file($seed_file, (">".$md5.".seed.iter_".$i."\n".$seed_seq));
...
line 2+: count for each bp, each position on one line
    ...
    --log_file        (string,         required)  file for log data
    --seed_file       (string,         required)  name for the output seed file
    --md5             (string,         required)  md5 of the bin 
    ...
    --uclust_id       (integer,     default = 0)  --id argument for uclust; 0 forces all reads into a single cluster
